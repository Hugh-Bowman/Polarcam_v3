...
DATASET_DIR = Path("40nm precision exposure relation final") / "pending"
...
def _collect_points(dataset_dir: Path) -> list[RodPoint]:
    ...
    for row in raw_points:
        ...
        theta_deg = _interp(np.asarray(stretched_curve["r"], dtype=np.float64), np.asarray(stretched_curve["theta_deg"], dtype=np.float64), r_mean)
        dtheta_dr_rad = _interp(np.asarray(stretched_curve["r"], dtype=np.float64), np.asarray(stretched_curve["dtheta_dr_rad"], dtype=np.float64), r_mean)
...
def _add_saturation_region(ax: plt.Axes) -> None:
    ...
    ax.axvspan(SATURATION_EXPOSURE_MS, right, facecolor="#d9d9d9", alpha=0.25, hatch="///", edgecolor="#999999")
    y0, y1 = ax.get_ylim()
    ax.text(
        SATURATION_EXPOSURE_MS + 0.5 * (right - SATURATION_EXPOSURE_MS),
        y0 + 0.92 * (y1 - y0),
        "At and beyond 0.4 ms rods saturate",
        ha="center",
        va="top",
        color="#555555",
    )
...
def main() -> None:
    ...
    points = _collect_points(dataset_dir)
    if not points:
    ...
    _plot_metric(
        ...
        title="40nm rods precision: phi uncertainty vs exposure",
    ...
    _plot_metric(
        ...
        title="40nm rods precision: theta uncertainty vs exposure",
    ...
    summary = {
        ...
        "theta_deg_range": [float(np.min([p.theta_deg for p in points])), float(np.max([p.theta_deg for p in points]))],
        ...
        "saturation_note": "Plots mark exposure >= 0.40 ms as saturation-onset region.",
        "simple_model": {
            "pixel_variance": "Var_xy(t) ≈ a + b/t + c/t^2 before saturation, assuming signal I ∝ exposure time t",
            "sigma_phi": "sigma_phi(t) ≈ (1/(2 r)) sqrt(a + b/t + c/t^2)",
            "sigma_theta": "sigma_theta(t) ≈ |dtheta/dr| sqrt(a + b/t + c/t^2)",
            "saturation_behavior": "Beyond saturation the 1/t improvement should stop and can reverse due to clipping bias.",
        },
    }
