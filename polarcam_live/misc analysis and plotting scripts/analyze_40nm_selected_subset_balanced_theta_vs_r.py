...
SELECTED_RECORDINGS_CSV = (
    DATASET_ROOT
    / "plots"
    / "good_vs_good_plus_previous_used_center015"
    / "combined_phi_uniform_optimized"
    / "combined_per_recording_summary.csv"
)
OUTPUT_DIR = DATASET_ROOT / "plots" / "balanced_phi_xy_points_selected_subset"
PHI_BIN_DEG = 5.0
RANGE_THRESHOLD = 1.0
RNG_SEED = 12345
N_BOOT = 250
...
def _selected_names(path: Path) -> set[str]:
    rows = csv.DictReader(path.open("r", encoding="utf-8", newline=""))
    return {str(row["rod"]).strip() for row in rows if str(row.get("rod", "")).strip()}


def _load_filtered_points(root: Path, selected: set[str]) -> tuple[np.ndarray, np.ndarray, int]:
    xs: list[np.ndarray] = []
    ys: list[np.ndarray] = []
    kept = 0
    for sub in ("good", "bad", "pending"):
        src = root / sub
        if not src.exists():
            continue
        for rod_dir in sorted([p for p in src.iterdir() if p.is_dir() and p.name in selected]):
            ...
            kept += 1
    if not xs:
        raise SystemExit("No selected recordings passed the range filter.")
...
def _sample_balanced_points(x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    for idx in idx_by_bin:
        sampled_idx.append(np.asarray(rng.choice(idx, size=n_per_bin, replace=False), dtype=np.int64))
...
def _bootstrap_theta_curve(r: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for _ in range(int(N_BOOT)):
        ...
        try:
            rb, tb = _fit_uniform_costheta_curve(r[idx])
...
def main() -> None:
    ...
    selected = _selected_names(Path.cwd() / SELECTED_RECORDINGS_CSV)
    out_dir = Path.cwd() / OUTPUT_DIR
    out_dir.mkdir(parents=True, exist_ok=True)
    x_all, y_all, kept = _load_filtered_points(root, selected)
    ...
    ax.fill_between(r_grid, theta_lo, theta_hi, color="#9467bd", alpha=0.22, label="Bootstrap 1 sigma band")
    ax.plot(r_grid, theta_center, color="#6f42c1", lw=2.3, label="Theta(r) empirical")
    ax.set_xlabel("r")
    ax.set_ylabel("theta (deg)")
    ax.set_title("Theta(r) from selected-subset balanced 40nm glycerol xy points")
    ...
    summary = {
        "selected_recordings_csv": str((Path.cwd() / SELECTED_RECORDINGS_CSV).resolve()),
        "output_dir": str(out_dir.resolve()),
        "selected_recording_names": int(len(selected)),
        "selected_recordings_passing_filter": int(kept),
        ...
        "n_boot": int(N_BOOT),
        "method": "Apply the same balanced-point method to only the recordings used in the original 40nm curve, after the same range_x > 1 and range_y > 1 filter.",
    }
