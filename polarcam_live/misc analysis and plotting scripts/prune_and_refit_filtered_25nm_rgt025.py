...
SOURCE_GOOD_DIR = Path("stationary rods 25nm 02072026") / "filtered_intensity_p98_50_to_400" / "good"
PRUNED_ROOT = Path("stationary rods 25nm 02072026") / "filtered_intensity_p98_50_to_400_sigma_theta_pruned"
PRUNED_GOOD_DIR = PRUNED_ROOT / "good"
OUTPUT_DIR = PRUNED_ROOT / "plots" / "pixel_error_c_over_I2_fit_r_gt_0p25"
CURVE_CSV = (
...
THETA_PRUNE_MIN_DEG = 30.0
SIGMA_THETA_PRUNE_MAX_DEG = 10.0
LOW_THETA_PRUNE_MAX_DEG = 40.0
LOW_THETA_INTENSITY_MAX = 35.0
...
def _load_xy_series(meta_path: Path) -> np.ndarray:
    ...
    arr = np.asarray(payload.get("xy_series"), dtype=np.float64)
    arr = arr[:, :2]
...
def _predict_using_actual_intensity(data: list[RodDatum], c_over_i2: float) -> dict[str, np.ndarray]:
    r_mean = np.asarray([d.r_mean for d in data], dtype=np.float64)
    dtheta_dr_deg = np.asarray([np.degrees(abs(d.dtheta_dr_rad)) for d in data], dtype=np.float64)
    ...
    sigma_xy = np.sqrt(float(c_over_i2) / (intensity_actual * intensity_actual))
    sigma_theta = dtheta_dr_deg * sigma_xy
    sigma_phi = np.degrees(sigma_xy / np.maximum(2.0 * r_mean, 1e-12))
    return {
...
def _copy_pruned_dataset(keep_rods: set[str]) -> None:
    PRUNED_GOOD_DIR.mkdir(parents=True, exist_ok=True)
    for existing in list(PRUNED_GOOD_DIR.iterdir()):
        if existing.is_dir() and existing.name not in keep_rods:
            shutil.rmtree(existing)
    for src in sorted([p for p in SOURCE_GOOD_DIR.iterdir() if p.is_dir()]):
        if src.name not in keep_rods:
            continue
        dst = PRUNED_GOOD_DIR / src.name
        if dst.exists():
            shutil.rmtree(dst)
...
def main() -> None:
    ...
    source_data = _load_data(Path.cwd() / SOURCE_GOOD_DIR)
    if not source_data:
        raise SystemExit("No rods found in source dataset.")

    stretched_curve, stretch_scale = _stretch_curve_for_dataset(curve, float(max(d.r_mean for d in source_data)))
    _apply_curve_to_data(source_data, stretched_curve)

    removed = []
    for d in source_data:
        remove_high_sigma = (
            d.theta_deg > THETA_PRUNE_MIN_DEG and d.sigma_theta_xy_only_deg > SIGMA_THETA_PRUNE_MAX_DEG
        )
        remove_low_theta_bright = (
            d.theta_deg < LOW_THETA_PRUNE_MAX_DEG and d.intensity_mean > LOW_THETA_INTENSITY_MAX
        )
        if remove_high_sigma or remove_low_theta_bright:
            removed.append(d)
    kept = [d for d in source_data if d not in removed]
    keep_rods = {d.rod for d in kept}
    _copy_pruned_dataset(keep_rods)

    fit_data = list(kept)
    if not fit_data:
        raise SystemExit("No rods remain for fitting after pruning.")
    fit = _fit(fit_data)
    pred_all = _predict(kept, fit["a_sin2"], fit["b_cos2"], fit["c_over_i2"])
    pred_actual_all = _predict_using_actual_intensity(kept, fit["c_over_i2"])

    rows = []
    for datum, intensity_model, sigma_xy_pred, sigma_theta_pred, sigma_phi_pred, sigma_xy_pred_actual, sigma_theta_pred_actual, sigma_phi_pred_actual in zip(
        kept,
        pred_all["intensity_model"],
        pred_all["sigma_xy"],
        pred_all["sigma_theta"],
        pred_all["sigma_phi"],
        pred_actual_all["sigma_xy"],
        pred_actual_all["sigma_theta"],
        pred_actual_all["sigma_phi"],
    ):
        rows.append(
            {
                "rod": datum.rod,
    ...
    with (out_dir / "pruned_filtered_25nm_r_gt_0p25_fitted_points.csv").open("w", encoding="utf-8", newline="") as fh:
    ...
    removed_rows = [
        {
            ...
            "sigma_theta_measured_deg": d.sigma_theta_xy_only_deg,
            "sigma_phi_measured_deg": d.sigma_phi_deg,
            "intensity_mean": d.intensity_mean,
            "remove_high_sigma_above_30deg": (
                d.theta_deg > THETA_PRUNE_MIN_DEG and d.sigma_theta_xy_only_deg > SIGMA_THETA_PRUNE_MAX_DEG
            ),
            "remove_bright_below_40deg": (
                d.theta_deg < LOW_THETA_PRUNE_MAX_DEG and d.intensity_mean > LOW_THETA_INTENSITY_MAX
            ),
        }
        for d in removed
    ]
    with (PRUNED_ROOT / "removed_rods.csv").open("w", encoding="utf-8", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(removed_rows[0].keys()) if removed_rows else ["rod", "r_mean", "theta_deg", "sigma_theta_measured_deg", "sigma_phi_measured_deg", "intensity_mean"])
        writer.writeheader()
        if removed_rows:
            writer.writerows(removed_rows)

    theta_all = np.asarray([d.theta_deg for d in kept], dtype=np.float64)
    inten_all = np.asarray([d.intensity_mean for d in kept], dtype=np.float64)
    sth_meas_all = np.asarray([d.sigma_theta_xy_only_deg for d in kept], dtype=np.float64)
    sth_pred_all = np.asarray([float(r["sigma_theta_pred_actual_intensity_deg"]) for r in rows], dtype=np.float64)
    sphi_meas_all = np.asarray([d.sigma_phi_deg for d in kept], dtype=np.float64)
    sphi_pred_all = np.asarray([float(r["sigma_phi_pred_actual_intensity_deg"]) for r in rows], dtype=np.float64)
    used_mask = np.ones(len(kept), dtype=bool)
    ...
    ax.scatter(theta_all[~used_mask], inten_all[~used_mask], s=34, alpha=0.55, color="#b8a6d9", edgecolors="none", label="Not used in fit")
    ax.scatter(theta_all[used_mask], inten_all[used_mask], s=40, alpha=0.85, color="#9467bd", edgecolors="none", label="Used in fit")
    ax.plot(theta_grid, I_grid, color="#111111", lw=2.0)
    ax.set_xlabel("theta (deg)")
    ax.set_ylabel("Mean pixel intensity I")
    ax.set_title("25nm rods, r>0.25 fit: intensity vs theta")
    ...
    fig, ax = plt.subplots(figsize=(7.2, 4.8))
    ax.scatter(theta_all[~used_mask], sth_meas_all[~used_mask], s=34, alpha=0.55, color="#8fb8df", edgecolors="none", label="Not used in fit")
    ax.scatter(theta_all[used_mask], sth_meas_all[used_mask], s=40, alpha=0.85, color="#1f77b4", edgecolors="none", label="Used in fit")
    ax.plot(theta_grid, sigma_theta_grid, color="#d62728", lw=2.0)
    ax.set_xlabel("theta (deg)")
    ax.set_ylabel("std dev theta (deg)")
    ax.set_title("25nm rods, r>0.25 fit: theta std vs theta")
    ax.grid(True, alpha=0.22)
    ax.legend(loc="best", frameon=False)
    fig.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(7.2, 4.8))
    ax.scatter(theta_all[~used_mask], sphi_meas_all[~used_mask], s=34, alpha=0.55, color="#97d497", edgecolors="none", label="Not used in fit")
    ax.scatter(theta_all[used_mask], sphi_meas_all[used_mask], s=40, alpha=0.85, color="#2ca02c", edgecolors="none", label="Used in fit")
    ax.plot(theta_grid, sigma_phi_grid, color="#ff7f0e", lw=2.0)
    ax.set_xlabel("theta (deg)")
    ax.set_ylabel("std dev phi (deg)")
    ax.set_title("25nm rods, r>0.25 fit: phi std vs theta")
    ax.grid(True, alpha=0.22)
    ax.legend(loc="best", frameon=False)
    fig.tight_layout()
    ...
    ax.set_title("25nm rods, r>0.25 fit: predicted vs actual theta std")
    ax.grid(True, alpha=0.22)
    ax.legend(loc="best", frameon=False)
    fig.tight_layout()
    ...
    ax.set_title("25nm rods, r>0.25 fit: predicted vs actual phi std")
    ax.grid(True, alpha=0.22)
    ax.legend(loc="best", frameon=False)
    fig.tight_layout()
    ...
    summary = {
        "source_good_dir": str((Path.cwd() / SOURCE_GOOD_DIR).resolve()),
        "pruned_good_dir": str(PRUNED_GOOD_DIR.resolve()),
        "output_dir": str(out_dir.resolve()),
        "curve_csv": str((Path.cwd() / CURVE_CSV).resolve()),
        "prune_rule": {
            "theta_deg_gt": THETA_PRUNE_MIN_DEG,
            "sigma_theta_deg_gt": SIGMA_THETA_PRUNE_MAX_DEG,
            "theta_deg_lt": LOW_THETA_PRUNE_MAX_DEG,
            "intensity_mean_gt": LOW_THETA_INTENSITY_MAX,
        },
        "n_source_rods": len(source_data),
        "n_removed_rods": len(removed),
        "n_kept_rods": len(kept),
        "n_rods_used_in_fit": len(fit_data),
        "r_mean_min_fit": None,
        "curve_r_stretch": {
            "dataset_r_max": float(max(d.r_mean for d in source_data)),
