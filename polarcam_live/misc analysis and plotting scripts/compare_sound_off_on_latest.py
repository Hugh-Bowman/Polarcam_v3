...
recs = [
    ("sound_off_assumed", base / "rod_x1558_y1166_20260722-115930_1784717970058907300"),
    ("sound_on_assumed", base / "rod_x1558_y1166_20260722-115950_1784717990016892600"),
]
out = base / "sound_off_vs_on_20260722-115930_115950_analysis"
...
def traces(rec):
    ...
    cx = ((int(meta.get('requested', {}).get('center', {}).get('x', roi.get('x',0)+7))) - int(roi.get('x',0))) / 2.0
    cy = ((int(meta.get('requested', {}).get('center', {}).get('y', roi.get('y',0)+7))) - int(roi.get('y',0))) / 2.0
    wraw = meta.get('requested', {}).get('window_raw', 11)
    cw = odd_window(float(wraw) / 2.0, min(next(iter(chans.values())).shape[1:]))
    channel_parts = []
    pix_total = 0
    for name in ['I0','I45','I135','I90']:
        tr, npix = centered_crop_mean(chans[name], cx, cy, cw)
        channel_parts.append(tr * npix)
        pix_total += npix
    channel_mean = np.sum(channel_parts, axis=0) / pix_total
    return {
        'fps': fps,
...
specs = []
for label, rec, t in data:
    f, pxx = spectrum(label, t['channel_mean'], t['fps'])
...
with open(out/'spectrum_peak_comparison.csv','w', newline='') as f:
    w = csv.DictWriter(f, fieldnames=['label','kind','frequency_hz','power_density'])
...
print('\nIntensity stats, channel_mean:')
for r in rows:
    if r['trace'] == 'channel_mean':
        print(r['label'], 'mean', round(r['mean'],3), 'p95-p5', round(r['p95_minus_p5'],3), 'pct', round(r['p95_minus_p5_percent_mean'],3), 'range', round(r['range'],3), 'std', round(r['std'],3))
print('\n640 region peaks and nearest 640 bins:')
