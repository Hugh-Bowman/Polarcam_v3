...
focus_rec=base/'rod_x1388_y1234_20260722-152208_1784730128149053600'
sound_rec=base/'rod_x1388_y1234_20260722-151938_1784729978865736100'
out=base/'latest_sound_on_vs_new_focus_pan_intensity_comparison_151938_152208'
...
def p98p2(y):
    p2,p98=np.percentile(y,[2,98]); return float(p98-p2),float(p2),float(p98)

def psd_summary(y,fps):
    yy=y-np.mean(y); nperseg=min(len(yy),4096)
    f,p=welch(yy,fs=fps,nperseg=nperseg,noverlap=min(nperseg//2,2048),scaling='density')
    m=(f>=1)&(f<=fps/2); idxs=np.flatnonzero(m); idx=idxs[np.argmax(p[m])]
    idx780=int(np.argmin(np.abs(f-780)))
    return {'dominant_peak_hz':float(f[idx]),'dominant_peak_psd':float(p[idx]),'nearest_780_hz':float(f[idx780]),'nearest_780_psd':float(p[idx780])}
focus=load(focus_rec); sound=load(sound_rec)

def make_plot(fname,title,key):
    fig,axs=plt.subplots(2,1,figsize=(12,7),constrained_layout=True,sharex=False,sharey=True)
    for ax,label,d,color in [(axs[0],'Focus pan',focus,'tab:blue'),(axs[1],'Sound on',sound,'tab:orange')]:
...
make_plot('raw_counts_focus_pan_vs_sound_on_angle_window_p98p2.png','Focus pan vs sound on, angle-window mean','channel')
make_plot('raw_counts_focus_pan_vs_sound_on_raw11_p98p2.png','Focus pan vs sound on, raw central 11x11 mean','raw11')
summary={'recordings':{'focus_pan':{'path':focus['path'],'fps':focus['fps'],'n_frames':focus['n_frames'],'duration_s':focus['duration_s']},'sound_on':{'path':sound['path'],'fps':sound['fps'],'n_frames':sound['n_frames'],'duration_s':sound['duration_s']}},'p98_minus_p2_counts':{'angle_window':{'focus_pan':p98p2(focus['channel'])[0],'sound_on':p98p2(sound['channel'])[0]},'raw11':{'focus_pan':p98p2(focus['raw11'])[0],'sound_on':p98p2(sound['raw11'])[0]}},'sound_spectrum_channel':psd_summary(sound['channel'],sound['fps']),'output_dir':str(out)}
...
print('angle_window_sound_p98p2',p98p2(sound['channel'])[0])
print('raw11_focus_p98p2',p98p2(focus['raw11'])[0])
print('raw11_sound_p98p2',p98p2(sound['raw11'])[0])
print('sound spectrum',psd_summary(sound['channel'],sound['fps']))
