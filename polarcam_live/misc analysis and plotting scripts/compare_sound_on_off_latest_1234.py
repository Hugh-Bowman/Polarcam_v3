...
recs = [
    ("sound_on_assumed", base / "rod_x1546_y1205_20260722-123421_1784720061771203700"),
    ("sound_off_assumed", base / "rod_x1546_y1205_20260722-123435_1784720075764483500"),
]
out = base / "sound_on_vs_off_20260722-123421_123435_analysis"
...
for label, rec, t in data:
    ...
    for freq in [640.0,640.65,1000.0,24.0,29.0,162.0,450.0,750.0]:
        nf,npow=nearest_power(f,pxx,freq); spec_rows.append({'label':label,'kind':f'nearest_{freq}_Hz','frequency_hz':nf,'power_density':npow})
...
for label, rec, t in data:
    tt=np.arange(len(t['channel_mean']))/t['fps']; ax.plot(tt,t['channel_mean'],lw=0.8,label=label)
ax.set_xlabel('Time (s)'); ax.set_ylabel('Mean intensity over angle window'); ax.set_title('Intensity trace: assumed sound on vs sound off'); ax.grid(alpha=.25); ax.legend()
fig.savefig(out/'intensity_time_on_vs_off.png',dpi=200); plt.close(fig)

for fname,xlim,title in [('spectrum_on_vs_off_1_820Hz.png',(1,820),'Spectrum: 1 to Nyquist'),('spectrum_on_vs_off_640Hz_region.png',(580,700),'Spectrum around 640 Hz'),('spectrum_on_vs_off_lowfreq.png',(1,250),'Low-frequency spectrum')]:
    fig, ax=plt.subplots(figsize=(11,5), constrained_layout=True)
...
summary={'assumption':'Two newest folders used; earlier labelled sound_on_assumed, later labelled sound_off_assumed from user wording.', 'recordings':[{'label':l,'path':str(r),'fps':t['fps'],'n_frames':t['n_frames'],'duration_s':t['duration_s']} for l,r,t in data], 'intensity_stats':rows, 'spectrum_checks':spec_rows, 'output_dir':str(out)}
