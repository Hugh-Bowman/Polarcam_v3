...
DEFAULT_TDMS = Path("/Users/hughbowman/Desktop/transfer 3107/reference rod apd vs cam/apd rod1.tdms")
DEFAULT_MAPPING = (3, 0, 1, 2)  # TDMS channel indices used as I0, I90, I45, I135.
DEFAULT_SCALES = (1.27, 1.0, 2.93, 2.82)  # I0, I90, I45, I135.
...
def block_average(values: np.ndarray, target_points: int) -> np.ndarray:
    arr = np.asarray(values, dtype=np.float64)
    n = int(arr.shape[0])
    target_points = max(1, min(int(target_points), n))
    if target_points == n:
        return arr.copy()
    edges = np.linspace(0, n, target_points + 1).astype(np.int64)
    return np.vstack([arr[edges[i] : edges[i + 1]].mean(axis=0) for i in range(target_points) if edges[i + 1] > edges[i]])
...
class ManualScalingXYApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("TDMS Manual Channel Scaling XY")
        ...
        self.points_var = tk.StringVar(value="3000")
        ...
        self.theta_model_var = tk.StringVar(value="hole+fresnel")
        ...
        self.scale_vars = [tk.StringVar(value=f"{scale:g}") for scale in DEFAULT_SCALES]
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="Averaged plot points").grid(row=1, column=2, sticky="w", pady=(8, 0))
        points_entry = ttk.Entry(top, textvariable=self.points_var, width=8)
        points_entry.grid(row=1, column=3, sticky="w", padx=(6, 18), pady=(8, 0))

        scale_frame = ttk.Frame(top)
        scale_frame.grid(row=1, column=4, columnspan=5, sticky="ew", pady=(8, 0))
        for i, label in enumerate(POL_LABELS):
            ttk.Label(scale_frame, text=f"{label} scale").grid(row=0, column=i * 2, sticky="e")
            entry = ttk.Entry(scale_frame, textvariable=self.scale_vars[i], width=9)
            entry.grid(row=0, column=i * 2 + 1, sticky="w", padx=(4, 10))
            entry.bind("<Return>", lambda _event: self.update_plot())

        ttk.Button(top, text="Update Plot", command=self.update_plot).grid(row=1, column=9, sticky="e", pady=(8, 0))
        ttk.Label(top, text="Theta model").grid(row=2, column=0, sticky="w", pady=(8, 0))
        ...
        theta_box.grid(row=2, column=1, sticky="w", padx=(6, 16), pady=(8, 0))
        ttk.Label(top, text="Start frame").grid(row=2, column=2, sticky="w", pady=(8, 0))
        start_entry = ttk.Entry(top, textvariable=self.start_frame_var, width=10)
        start_entry.grid(row=2, column=3, sticky="w", padx=(6, 12), pady=(8, 0))
        ttk.Label(top, text="Num frames").grid(row=2, column=4, sticky="w", pady=(8, 0))
        frame_count_entry = ttk.Entry(top, textvariable=self.frame_count_var, width=10)
        frame_count_entry.grid(row=2, column=5, sticky="w", padx=(6, 12), pady=(8, 0))
        ttk.Label(top, text="Sin hist bin").grid(row=2, column=6, sticky="w", pady=(8, 0))
        bin_entry = ttk.Entry(top, textvariable=self.sin_bin_width_var, width=8)
        bin_entry.grid(row=2, column=7, sticky="w", padx=(6, 12), pady=(8, 0))
        ttk.Button(top, text="Reset Scales", command=self.reset_scales).grid(row=2, column=9, sticky="e", pady=(8, 0))
    ...
    def load_tdms(self, path: Path) -> None:
        ...
        self.frame_count_var.set(str(raw.shape[0]))
        self.update_plot()

    def reset_scales(self) -> None:
        for var, scale in zip(self.scale_vars, DEFAULT_SCALES):
            var.set(f"{scale:g}")
        self.update_plot()

    def _current_inputs(self) -> tuple[tuple[int, int, int, int], np.ndarray, int, int, int, float]:
        mapping = parse_mapping(self.mapping_var.get())
        scales = np.asarray([float(var.get()) for var in self.scale_vars], dtype=np.float64)
        if not np.all(np.isfinite(scales)):
            raise ValueError("All scale values must be finite numbers.")
        if np.any(scales <= 0.0):
            raise ValueError("All scale values must be positive.")
        points = int(float(self.points_var.get()))
        if points <= 0:
            raise ValueError("Plot points must be positive.")
        total = int(self.raw.shape[0]) if self.raw is not None else 0
        start = int(float(self.start_frame_var.get() or "0"))
        start = max(0, min(start, total))
        count_text = self.frame_count_var.get().strip()
        count = total - start if count_text == "" else int(float(count_text))
        if count <= 0:
        ...
        return mapping, scales, points, start, end, bin_width
    ...
    def update_plot(self) -> None:
        ...
        try:
            mapping, scales, points, start, end, sin_bin_width = self._current_inputs()
        except Exception as exc:
        ...
        averaged = block_average(mapped, points)
        scaled = averaged * scales.reshape(1, 4)
        xy = xy_from_intensities(scaled)
        ...
        self.ax_xy.set_title("Scaled XY")
        ...
        self.figure.suptitle(f"{self.loaded_path.name if self.loaded_path else ''} | mapping {mapped_names}", fontsize=10)
        ...
        self.status_var.set(
            " | ".join(
                [
                    ...
                    f"selected frames=start {start}, count {selected_frames} ({start}:{end})",
                    f"scales I0/I90/I45/I135={scales[0]:.6g}, {scales[1]:.6g}, {scales[2]:.6g}, {scales[3]:.6g}",
